 ...
 *
 */
@ReqActionAnnotation(cmbBizType = TrustBizType.BCCK, cmbCategory = TrustCategory.TRADE, thirdType = ThirdType.CMBPAY)
@Component(value = "com.zendaimoney.trust.channel.action.account.BcckTradeAction")
public class BcckTradeAction extends TradeActionAbstract {

	private static final LogPrn logger = new LogPrn(BcckTradeAction.class);
		...
		try {
			if (Constants.CmbStatus.CMBMB99.getCode().equals(headerRetCode)) {
				...
				if (Constants.CmbStatus.CMBMB99.getCode().equals(bodyRetCode)) {
					...
					//GeneralOper generalOper = new GeneralOper(message.getTradeSn(), Constants.OPER_STATUS_SUCCESS,CalendarUtils.getFormatNow(), bodyMsg);
					// 批量更新操作对象
					...
					BeanUtils.copyNotNullProperties(response, attachmentResponse);
					...
					response.setMsg(StringUtils.isBlank(bodyMsg) ? cmbStatus != null ? cmbStatus.getDesc()
