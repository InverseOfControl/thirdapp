...
import com.zendaimoney.trust.channel.entity.batch.FrozTop;
...
@ReqActionAnnotation(thirdType = ThirdType.CMBPAY, cmbBizType = TrustBizType.FROZ, cmbCategory = TrustCategory.BATCH_TRADE)
@Component("com.zendaimoney.trust.channel.action.vtrade.FrozBatchAction")
public class FrozBatchAction extends BatchActionAbstract {

	private static final LogPrn logger = new LogPrn(FrozBatchAction.class);
	...
	protected List preProcess(TrustBizReqVo trustBizReqVo) throws PlatformException {
		...
		// 报文首行对象
		FrozTop frozTop = new FrozTop(requestData.size(), calculateRechargeAmountSum(requestData));
		// 添加报文首行内容
		messages.add(ConvertUtils.objToMessage(frozTop));
		
		FrozDetailReq frozDetailReq = null;
		// 遍历数据集
		for (int i = 0; i < requestData.size(); i++) {
			// 转换为明细对象
			frozDetailReq = (FrozDetailReq)requestData.get(i);
			
			// 将单条记录转换为每行报文
			messages.add(ConvertUtils.objToMessage(frozDetailReq));
		}
...
//			ConvertUtils.messageToObj(list.get(0).toString(), usraTop);
			// 明细对象载体
			FrozDetailReq record = new FrozDetailReq();
	...
	private BigDecimal calculateRechargeAmountSum(List requestData) {
		...
		for (int i = 0; i < requestData.size(); i++) {
			...
			// 统计充值总额
			amountSum = amountSum.add(frozDetailReq.getAmount());
		}
		return amountSum;
	}
}
