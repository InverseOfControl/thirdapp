 ...
 *
 */
public abstract class TradeActionAbstract {

	private static final LogPrn logger = new LogPrn(TradeActionAbstract.class);
	...
	@Autowired
	protected SocketSenderImpl socketSender;
	 ...
	 * @return
	 */
	protected Response communicationFailHandler(Message message,TrustBizReqVo bizReqVo) {
		// 返回调用方为交易失败
	...
	public Response execute(TrustBizReqVo bizReqVo) {
		...
		try {
			...
			logger.info("channel最终响应结果:[code:"+response.getCode()+",msg:"+response.getMsg()+
					(response.getBankRepCode()!=null?",bankRepCode:"+response.getBankRepCode()+"]":"]"));
		} catch (PlatformException e) {
