...
import com.zendaimoney.trust.channel.pub.vo.req.UnfrReq;
import com.zendaimoney.trust.channel.pub.vo.resp.UnfrResp;
...
import com.zendaimoney.trust.channel.util.BigDecimalFormatUtil;
 ...
 *
 */
@ReqActionAnnotation(thirdType = ThirdType.CMBPAY, cmbBizType = TrustBizType.UNFR, cmbCategory = TrustCategory.TRADE)
@Component("com.zendaimoney.trust.channel.action.vtrade.UnfrTradeAction")
public class UnfrTradeAction extends TradeActionAbstract {

	// 日志工具类
	private static final LogPrn logger = new LogPrn(UnfrTradeAction.class);
	...
	protected MessageDto parseMessage(Message message, TrustBizReqVo bizReqVo)
		...
		try {
			...
			if (Constants.CmbStatus.CMBMB99.getCode().equals(header.getRetCode())) {
				...
				unfrResp = new UnfrResp();
				// 解析响应报文
				ConvertUtils.messageToObj(message, unfrResp);
			}
			
			// 解析完之后 将报文头和报文体赋值在报文传输对象上
			messageDto = new MessageDto(header, unfrResp);
		} catch (Exception e) {
			logger.error("====", e);
...
//	}
	...
	protected Response communicationSuccessHandler(MessageDto messageDto,
		...
		UnfrResp resp = (UnfrResp) messageDto.getRespVO();
		Request request = null;
		...
		try {
			...
			if (Constants.CmbStatus.CMBMB99.getCode().equals(retCode)) {
				...
				if (Constants.CmbStatus.CMBMB99.getCode().equals(tradeCode)) {
					...
					AcctCommand acc = new AcctCommand(bizReqVo.getTradeFlow(), Constants.OPER_STATUS_SUCCESS,"", resp.getMsg(), 
							CalendarUtils.getFormatNow(),resp.getBankAccountDate()==null?"":resp.getBankAccountDate(),
							resp.getFrozAmount()!=null?BigDecimalFormatUtil.bigDeclimalToString(resp.getFrozAmount()):null);
					// 业务更新处理对象
					finalResultHandler(tradeResult, acc, message, response, bizReqVo);
					
					// 响应对象泛型
					AttachmentResponse<UnfrResp> attachmentResponse = new AttachmentResponse<UnfrResp>();
					// 赋值响应结果信息
					...
					// 账户资金对象
					AcctCommand acc = new AcctCommand(bizReqVo.getTradeFlow(), Constants.OPER_STATUS_FAIL,"", resp.getMsg(), CalendarUtils.getFormatNow(),
							resp.getBankAccountDate()==null?"":resp.getBankAccountDate(),resp.getFrozAmount()!=null?BigDecimalFormatUtil.bigDeclimalToString(resp.getFrozAmount()):null);
				...
				// 账户资金对象
				AcctCommand acc = new AcctCommand(bizReqVo.getTradeFlow(), Constants.OPER_STATUS_FAIL,"", response.getMsg(), CalendarUtils.getFormatNow(),null,null);
				// 业务更新处理对象
				finalResultHandler(tradeResult, acc, message, response, bizReqVo);
			}
