 ...
 *
 */
@ReqActionAnnotation(thirdType = ThirdType.CMBPAY, cmbBizType = TrustBizType.USRE, cmbCategory = TrustCategory.TRADE)
@Component("com.zendaimoney.trust.channel.action.user.UsreTradeAction")
public class UsreTradeAction extends TradeActionAbstract {
	
	// 日志工具类
	private static final LogPrn logger = new LogPrn(UsreTradeAction.class);
	...
	protected TrustBizReqVo preProcess(TrustBizReqVo vo) throws PlatformException {
		...
		UsreReq usreReq = null;
		Request request = null;
		try {
			usreReq = (UsreReq) vo;
			
			// TPP证件类型映射到第三方证件类型
			usreReq.setIdType(ThirdPPCacheContainer.tppIdTypeToThirdIdTypeMap
					.get(vo.getThirdType().getCode() + usreReq.getIdType()));
	...
	protected MessageDto parseMessage(Message message, TrustBizReqVo bizReqVo)
		...
		try {
			...
			if (Constants.CmbStatus.CMBMB99.getCode().equals(header.getRetCode())) {
				...
				usreResp = new CommonResp();
				// 解析响应报文
				ConvertUtils.messageToObj(message, usreResp);
			}
			
			// 解析完之后 将报文头和报文体赋值在报文传输对象上
			messageDto = new MessageDto(header, usreResp);
		} catch (Exception e) {
			logger.error("====", e);
...
//	}
	...
	protected Response communicationSuccessHandler(MessageDto messageDto, Message message,
		...
		try {
			...
			// 首先判断报文头 响应码是否为 CMBMB99，响应成功 ,交易返回码是否成功(交易状态码为 CMBMB99)
