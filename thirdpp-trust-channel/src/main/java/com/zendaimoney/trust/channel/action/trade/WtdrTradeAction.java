 ...
 *
 */
@ReqActionAnnotation(thirdType = ThirdType.CMBPAY, cmbBizType = TrustBizType.WTDR, cmbCategory = TrustCategory.TRADE)
@Component("com.zendaimoney.trust.channel.action.trade.WtdrTradeAction")
public class WtdrTradeAction extends TradeActionAbstract {

	// 日志工具类
	private static final LogPrn logger = new LogPrn(WtdrTradeAction.class);
	...
	protected TrustBizReqVo preProcess(TrustBizReqVo vo)
		...
		WtdrReq wtdrReq = null;
		Request request = null;
		try {
			// 请求对象转换
			wtdrReq = (WtdrReq) vo;
			wtdrReq.setBankName(ThirdPPCacheContainer.tppBankCodeToThirdBankNameMap.get(
					vo.getThirdType().getCode()+wtdrReq.getBankName()));
			wtdrReq.setBankSubName(ThirdPPCacheContainer.tppBankCodeToThirdBankNameMap.get(
					vo.getThirdType().getCode()+wtdrReq.getBankSubName()));
		} catch (Exception e) {
	...
	protected MessageDto parseMessage(Message message, TrustBizReqVo bizReqVo)
		...
		try {
			...
			if (Constants.CmbStatus.CMBMB99.getCode().equals(header.getRetCode())) {
				...
				wtdrResp = new WtdrResp();
				// 解析响应报文
				ConvertUtils.messageToObj(message, wtdrResp);
			}
			
			// 解析完之后 将报文头和报文体赋值在报文传输对象上
			messageDto = new MessageDto(header, wtdrResp);
		} catch (Exception e) {
			logger.error("====", e);
	...
	protected Response communicationSuccessHandler(MessageDto messageDto,
		...
		WtdrResp resp = (WtdrResp) messageDto.getRespVO();
		Request request = null;
		...
		try {
			...
			if (Constants.CmbStatus.CMBMB99.getCode().equals(retCode)) {
				...
				if (Constants.CmbStatus.CMBMB99.getCode().equals(tradeCode)) {
					...
					response.setMsg(Constants.CmbConstants.TRADE_STATE_MIDDLE.getDesc());
					...
					// 提现处理对象
					cashDraw = new CashDraw(bizReqVo.getTradeFlow(), Constants.DRAW_STATUS_MIDDLE, resp.getMsg(), null, 
							resp.getFreezeFlow(), null,resp.getBankAccountDate()==null?"":resp.getBankAccountDate(),
							resp.getAdvanceAmount()!=null?resp.getAdvanceAmount():null);
					// 业务更新处理对象
					midResultHandler(tradeResult,cashDraw,message,response,bizReqVo);
					...
					// 提现处理对象
					cashDraw = new CashDraw(bizReqVo.getTradeFlow(), Constants.DRAW_STATUS_MIDDLE, resp.getMsg(), null, resp.getFreezeFlow(), null,resp.getBankAccountDate()==null?"":resp.getBankAccountDate(),resp.getAdvanceAmount()!=null?resp.getAdvanceAmount():null);
					// 业务更新处理对象
					midResultHandler(tradeResult,cashDraw,message,response,bizReqVo);
					...
					// 提现处理对象
					cashDraw = new CashDraw(bizReqVo.getTradeFlow(), Constants.DRAW_STATUS_FAIL, resp.getMsg(), null, resp.getFreezeFlow(), null,resp.getBankAccountDate()==null?"":resp.getBankAccountDate());
					// 业务更新处理对象
				...
				// 提现处理对象
				cashDraw = new CashDraw(bizReqVo.getTradeFlow(), Constants.DRAW_STATUS_FAIL, response.getMsg());
				// 业务更新处理对象
				finalResultHandler(tradeResult, cashDraw, message, response, bizReqVo);
			}
	 ...
	 * @param response
	 */
	private void midResultHandler(TradeResult tradeResult, CashDraw cashDraw, Message message, Response response,
			TrustBizReqVo bizReqVo) {
		try {
			tradeResultService.finalResultHandler(tradeResult, cashDraw);
		} catch (Exception e) {
	...
	/*private void sendNotifyQuery(TrustBizReqVo bizReqVo) {
		...
		try {
			...
			WtdrReq wtdrReq = (WtdrReq) bizReqVo;
			
			BeanUtils.copyProperties(cashDraw, wtdrReq);
