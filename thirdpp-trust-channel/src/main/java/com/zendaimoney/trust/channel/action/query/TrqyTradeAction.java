...
import com.zendaimoney.trust.channel.pub.vo.resp.TrqyResp;
 ...
 *
 */
@ReqActionAnnotation(thirdType = ThirdType.CMBPAY, cmbCategory = TrustCategory.TRADE, cmbBizType = TrustBizType.TRQY)
@Component(value = "com.zendaimoney.trust.channel.action.query.TrqyTradeAction")
public class TrqyTradeAction extends TradeActionAbstract{

	private static final LogPrn logger = new LogPrn(TrqyTradeAction.class);
	...
	protected MessageDto parseMessage(Message message, TrustBizReqVo bizReqVo) throws PlatformException {
		...
		TrqyResp trqyResp = null;
		try {
			Header header = parseHeadMessage(message, bizReqVo);
			if (Constants.CmbStatus.CMBMB99.getCode().equals(header.getRetCode())) {
				trqyResp = new TrqyResp();
				ConvertUtils.messageToObj(message, trqyResp);
			}
			messageDto = new MessageDto(header, trqyResp);
		} catch (Exception e) {
			logger.error("=========", e);
	...
	protected Response communicationSuccessHandler(MessageDto messageDto, Message message, TrustBizReqVo bizReqVo) {
		...
		TrqyResp trqyResp = (TrqyResp) messageDto.getRespVO();
		Request request = null;
		...
		try {
			...
			String bodyRetCode = (trqyResp != null ? trqyResp.getRetCode() : "");
			String bodyMsg = (trqyResp != null ? trqyResp.getOriginalTradeDesc() : "");
			...
			if (Constants.CmbStatus.CMBMB99.getCode().equals(headerRetCode)) {
				if (Constants.CmbStatus.CMBMB99.getCode().equals(bodyRetCode)) {
					...
					tradeResult = new TradeResult(Constants.CMB_PAY_SYS_NO,
							...
							Constants.CmbConstants.TRADE_STATE_SUCCESS.getCode(), message.getChannelReqId());
					...
					AttachmentResponse<TrqyResp> ap = new AttachmentResponse<TrqyResp>();
					BeanUtils.copyNotNullProperties(response, ap);
					ap.setAttachment(trqyResp);
					return ap;
				} else {
