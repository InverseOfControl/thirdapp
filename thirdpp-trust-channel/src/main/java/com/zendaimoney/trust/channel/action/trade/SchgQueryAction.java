...
/**
 * 自助转账充值请求(SCHG)Action-查询
 * @author mencius
 *
 */
@ReqActionAnnotation(thirdType = ThirdType.CMBPAY, cmbBizType = TrustBizType.SCHG, cmbCategory = TrustCategory.QUERY)
@Component("com.zendaimoney.trust.channel.action.trade.SchgQueryAction")
public class SchgQueryAction extends TradeActionAbstract {

	// 日志工具类
	private static final LogPrn logger = new LogPrn(SchgQueryAction.class);
	...
	protected MessageDto parseMessage(Message message, TrustBizReqVo bizReqVo)
		...
		try {
			...
			if (Constants.CmbStatus.CMBMB99.getCode().equals(header.getRetCode())) {
				...
				schgResp = new SchgResp();
				// 解析响应报文
				ConvertUtils.messageToObj(message, schgResp);
			}
			
			// 解析完之后 将报文头和报文体赋值在报文传输对象上
			messageDto = new MessageDto(header, schgResp);
		} catch (Exception e) {
			logger.error("====", e);
...
//	}
	...
	protected Response communicationSuccessHandler(MessageDto messageDto,
		...
		SchgResp resp = (SchgResp) messageDto.getRespVO();
		Request request = null;
		...
		try {
			...
			// 将招商银行返回交易状态赋予响应结果对象内
			response.setBankRepCode(tradeCode);
