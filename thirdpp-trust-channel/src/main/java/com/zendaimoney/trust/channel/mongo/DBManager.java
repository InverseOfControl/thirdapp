...
import com.zendaimoney.trust.channel.conf.MongoConfig;
import com.zendaimoney.trust.channel.conf.SystemConfig;
import com.zendaimoney.trust.channel.util.ConfigUtil;
import com.zendaimoney.trust.channel.util.MongoDBUtils;
...
public class DBManager {
	...
	static {
		...
		if (!MongoDataSource.dbMap.containsKey(MongoDataSource.DEFAULT)) {
			...
			logger.info("TPPGLOBALCONFIGPATH: " + SystemConfig.globalConfigPath);
			
			String path = "" ;
			if (StringUtils.isEmpty(SystemConfig.globalConfigPath)) {
				
				path = Thread.currentThread().getContextClassLoader().getResource("conf/mongo.properties").getPath();
			} else {
				File mongoFile = new File(SystemConfig.globalConfigPath + File.separator + "mongo.properties");
