...
/**
 * 用户开户处理Action-查询(DEMO)
 * @author mencius
 *
 */
@ReqActionAnnotation(thirdType = ThirdType.CMBPAY, cmbBizType = TrustBizType.USRA, cmbCategory = TrustCategory.QUERY)
@Component("com.zendaimoney.trust.channel.action.user.UsraQueryAction")
public class UsraQueryAction extends TradeActionAbstract {
	
	// 日志工具类
	private static final LogPrn logger = new LogPrn(UsraQueryAction.class);
	...
	protected TrustBizReqVo preProcess(TrustBizReqVo vo) throws PlatformException {
		...
		UsraReq usraReq = new UsraReq();
		
		
		try {
			BeanUtils.copyProperties(usraReq, vo);
			// 以下赋值均需从数据库内获取，暂作测试使用
			usraReq.setUserNo("1000005"); // 用户号
			usraReq.setUserName("mencius"); // 用户姓名
			usraReq.setUserType("1"); // 用户类型
			usraReq.setIdNo("411421197506123370"); // 证件号码
			usraReq.setIdType("0"); // 证件类型
			usraReq.setMobile("18221374887"); // 手机号码
