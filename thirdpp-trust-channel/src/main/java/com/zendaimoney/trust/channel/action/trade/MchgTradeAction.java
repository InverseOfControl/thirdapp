...
/**
 * 平台充值请求(MCHG)Action-交易
 * @author 00233197
 *
 */
@ReqActionAnnotation(thirdType = ThirdType.CMBPAY, cmbBizType = TrustBizType.MCHG, cmbCategory = TrustCategory.TRADE)
@Component("com.zendaimoney.trust.channel.action.trade.MchgTradeAction")
public class MchgTradeAction extends TradeActionAbstract {

	// 日志工具类
	private static final LogPrn logger = new LogPrn(MchgTradeAction.class);
	...
	/*private void sendNotifyQuery(TrustBizReqVo bizReqVo) {
		...
		try {
			...
			BeanUtils.copyProperties(recharge, mchgReq);
