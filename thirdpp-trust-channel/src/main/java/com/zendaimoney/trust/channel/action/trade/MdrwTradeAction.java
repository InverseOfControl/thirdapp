...
/**
 * 平台提现请求(MDRW)Action-交易
 * @author 00233197
 *
 */
@ReqActionAnnotation(thirdType = ThirdType.CMBPAY, cmbBizType = TrustBizType.MDRW, cmbCategory = TrustCategory.TRADE)
@Component("com.zendaimoney.trust.channel.action.trade.MdrwTradeAction")
public class MdrwTradeAction extends TradeActionAbstract {

	// 日志工具类
	private static final LogPrn logger = new LogPrn(MdrwTradeAction.class);
	...
	protected Response communicationSuccessHandler(MessageDto messageDto,
		...
		try {
			...
			if (Constants.CmbStatus.CMBMB99.getCode().equals(retCode)) {
				...
				// 提现处理对象
				cashDraw = new CashDraw(bizReqVo.getTradeFlow(), Constants.DRAW_STATUS_FAIL, response.getMsg(), null, null, null,null);
				// 业务更新处理对象
				finalResultHandler(tradeResult, cashDraw, message, response, bizReqVo);
			}
