...
import com.zendaimoney.thirdpp.channel.dto.req.kjtpay.collect.trade.CollectReq;
import com.zendaimoney.thirdpp.channel.dto.resp.RespDto;
import com.zendaimoney.thirdpp.channel.dto.resp.kjtpay.collect.trade.CollectResp;
 ...
 * 
 */
@ReqActionAnnotation(thirdType = ThirdType.KJTPAY, bizType = BizType.BROKER_COLLECT, channelCategory = ChannelCategory.TRADE)
@Component("com.zendaimoney.thirdpp.channel.action.kjtpay.CollectAction")
public class CollectAction extends Action {
	...
	protected Message buildMessage(ReqDto dto, BizReqVo vo)
		...
		try {
			...
			//银行卡号加密
			String cardNo = KjtpayUtil.encryptData(channelInfo.getMerchantNo(),keyFilePath,certFilePath,channelInfo.getCertPwd(),req.getCard_no(),req.get_input_charset());
			//证件号加密
			String certificatesNumber = KjtpayUtil.encryptData(channelInfo.getMerchantNo(),keyFilePath,certFilePath,channelInfo.getCertPwd(),req.getCertificates_number(),req.get_input_charset());
			req.setCard_no(cardNo);
			req.setCertificates_number(certificatesNumber);
			
			// 请求报文，需要加密
			message.setRequestInfo(req.encode());
			
			//清除未加密字段值			
			req.setCertificatesNumberOpen(null);
			req.setCardNoOpen(null);
	...
	protected Response communicationSuccessHandler(RespDto dto,
		...
		try {
			...
			// 通道返回流水号
			String payTransFlow = res.getInner_trade_no()!=null?res.getInner_trade_no():"";

			// body通道返回码
			String bodyRetCode = res.getResult();
			...
			if((StringUtils.isNotEmpty(retCode) && Constants.KjtpayConstants.RESPONSE_ERR_CODE.getCode().equals(retCode)) ||
					(Constants.KjtpayConstants.RESPONSE_SUC_CODE.getCode().equals(retCode) && "false".equals(bodyRetCode))){
				// 备注如果存在，拼入返回描述
				...
				response.setCode(Constants.TppConstants.TRADE_STATE_FAILER
						.getCode());
				// 设置第三方通道返回错误信息
				response.setMsg(StringUtils.substring(retMsg,0, 150));
				// 交易记录状态
				updateCollectInfo = new CollectInfo(message.getTradeSn(),
						Constants.TppConstants.TRADE_STATE_FAILER.getCode(),
						payTransFlow, StringUtils.substring(retMsg, 0, 150), errorCode,
				...
				tradeResult = new TradeResult(res.getThirdType().getCode(),
						message.getTradeSn(), payTransFlow, res.getBizType()
								.getCode(), errorCode,StringUtils.substring(retMsg, 0, 150),
						Constants.TppConstants.TRADE_STATE_FAILER.getCode(),
	...
	private void updateCollectMerId(ReqDto dto) {
		CollectReq req = (CollectReq)dto;
		CollectInfo collectInfo = new CollectInfo(req.getOuter_trade_no(),req.getPartner_id());
		try {
