package com.zendaimoney.thirdpp.channel.action.baofoopay;
...
import com.zendaimoney.thirdpp.channel.communication.baofoopay.BaofoopayCommunication;
import com.zendaimoney.thirdpp.channel.dto.req.ReqDto;
import com.zendaimoney.thirdpp.channel.dto.req.baofoopay.collect.query.CollectQueryReq;
import com.zendaimoney.thirdpp.channel.dto.resp.RespDto;
import com.zendaimoney.thirdpp.channel.dto.resp.baofoopay.collect.query.CollectQueryResp;
 ...
 * 
 */
@ReqActionAnnotation(thirdType = ThirdType.BAOFOOPAY, bizType = BizType.BROKER_COLLECT, channelCategory = ChannelCategory.QUERY)
@Component("com.zendaimoney.thirdpp.channel.action.baofoopay.CollectQueryAction")
public class CollectQueryAction extends Action {
	...
	@Autowired
	private BaofoopayCommunication baofoopayCommunication;
	...
	protected Message process(BizReqVo vo, ReqDto dto) throws PlatformException {
		...
		// 把报文发送第三方
		try {
			message = baofoopayCommunication.sendMessageByHttp(message);
		} catch (Exception e) {
			logger.error("=================", e);
			// 设置http请求发送弄个异常状态码
	...
	protected Response respHandler(Message message, BizReqVo bizReqVo)
		...
		if (Constants.CommunicationStatus.HTTP_OK_STATUS.getCode().equals(
			...
			try {
				// 解密
				message.setResponseInfo(BaofoopayUtil.verifySign(message.getResponseInfo(), ""));
				logger.info("baofoopay query response info:"+message.getResponseInfo());
	...
	protected Message buildMessage(ReqDto dto, BizReqVo vo)
		...
		try {
			...
			// 报文请求地址
			message.setUrl(ConfigUtil.getInstance().getBaofoopayConfig()
					.getCollectingQrytransUrl());

			// 签名之后的报文
			String sign = BaofoopayUtil.Sign(req.getDataContent().encode(), channelInfo);
			req.setSign(sign);
			...
			req.setSign(null);
			req.setDataContent(null);

			// 请求参数对象Map
			Map<String, String> paraMap = BaofoopayUtil.xmlToMap(req.encode());
			paraMap.put("data_content",sign);
	...
	protected Response communicationSuccessHandler(RespDto dto,
		...
		// 获取所有中间态   [I,FF]
		String[] middleStatus = Constants.BaofoopayConstants.QUERY_STATE_MID.getCode().split(",");
		...
		try {
			...
			// 交易状态
			String bodyRetCode = res.getOrderStat();
			// 返回状态设置为：交易中间态
			if (StringUtils.isEmpty(bodyRetCode) || middleStatus[0].equals(bodyRetCode) || 
					(middleStatus[1].equals(bodyRetCode) && !Constants.BaofoopayConstants.QUERY_NONEXIST_CODE.getCode().equals(retCode))) {
				// 返回状态为-交易中间态
				response.setCode(Constants.TppConstants.TRADE_STATE_MIDDLE.getCode());
				
				// 如果交易通道返回成功(info和body中状态码都成功才算成功)
			} else if (Constants.BaofoopayConstants.QUERY_STATE_SUCC.getCode().equals(bodyRetCode)) {
				// 返回状态为-交易成功
				response.setCode(Constants.TppConstants.TRADE_STATE_SUCCESS.getCode());
				// 设置银行返回码
				response.setBankRepCode(bodyRetCode);
				
				// 其他定义交易失败   order_stat=F || order_stat=FF and red_code(应答码)=BF00128
			} else if(Constants.BaofoopayConstants.QUERY_STATE_ERROR.getCode().equals(bodyRetCode) ||   
					(middleStatus[1].equals(bodyRetCode) && Constants.BaofoopayConstants.QUERY_NONEXIST_CODE.getCode().equals(retCode))){
