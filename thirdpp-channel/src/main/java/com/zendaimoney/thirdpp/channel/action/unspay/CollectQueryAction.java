package com.zendaimoney.thirdpp.channel.action.unspay;
...
import com.zendaimoney.thirdpp.channel.communication.unspay.HttpUtil;
...
import com.zendaimoney.thirdpp.channel.dto.resp.unspay.collect.query.CollectQueryResp;
...
import com.zendaimoney.thirdpp.channel.util.unspay.UnspayUtil;
 ...
 * 
 */
@ReqActionAnnotation(thirdType = ThirdType.UNSPAY, bizType = BizType.BROKER_COLLECT, channelCategory = ChannelCategory.QUERY)
@Component("com.zendaimoney.thirdpp.channel.action.unspay.CollectQueryAction")
public class CollectQueryAction extends Action {
	...
	protected Message process(BizReqVo vo, ReqDto dto) throws PlatformException {
		...
		// 把报文发送第三方
		try {
			message = HttpUtil.post(message,true);
		} catch (Exception e) {
			logger.error("=================", e);
			// 设置http请求发送弄个异常状态码
	...
	protected Message buildMessage(ReqDto dto, BizReqVo vo)
		...
		try {
			...
			// 报文请求地址
			message.setUrl(ConfigUtil.getInstance().getUnspayConfig()
					.getCollectingQrytransUrl());

			req = UnspayUtil.signQueryMsg(req,vo,channelInfo);
			...
			String jsonstr = JSONHelper.bean2json(req);
			paraMap.put("json",jsonstr);
	...
	protected Response communicationSuccessHandler(RespDto dto,
		...
		try {
			// 通道响应码
			String retCode = res.getResultCode();
			// 通道返回信息
			String retMsg = res.getResultMsg();
			
			// 交易状态
			String bodyRetCode = res.getStatus();
			// 交易描述
			String tradeMsg = res.getDesc();

			String[] queryErrorCode = Constants.UnspayConstants.QUERY_RESPONSE_ERROR_CODE
					.getCode().split(",");
			if(StringUtils.isEmpty(retCode)){
				// 返回状态为-交易中间态
				response.setCode(Constants.TppConstants.TRADE_STATE_MIDDLE
						.getCode());
			}else if(Arrays.asList(queryErrorCode).contains(retCode)){
				// 返回状态为-交易失败
				response.setCode(Constants.TppConstants.TRADE_STATE_FAILER
						.getCode());

				// 设置第三方通道返回错误信息
				if (StringUtils.isBlank(tradeMsg)) {
					tradeMsg = retMsg;
				}
				response.setMsg((tradeMsg != null ? StringUtils.substring(
						tradeMsg, 0, 150) : ""));
				// 设置银行返回码
			...
			}else if(!Constants.UnspayConstants.RESPONSE_CODE.getCode().equals(retCode)){
				// 返回状态为-交易中间态
				response.setCode(Constants.TppConstants.TRADE_STATE_MIDDLE
						.getCode());
			}else{
				// 返回状态设置为：交易中间态
				if (StringUtils.isEmpty(bodyRetCode) ||
						Constants.UnspayConstants.TRADE_STATE_MID.getCode().equals(bodyRetCode)) {
					// 返回状态为-交易中间态
					response.setCode(Constants.TppConstants.TRADE_STATE_MIDDLE
							.getCode());

				} else if (Constants.UnspayConstants.RESPONSE_CODE.getCode().equals(retCode) &&
						Constants.UnspayConstants.TRADE_STATE_SUCC.getCode()
								.equals(bodyRetCode)) {
					// 返回状态为-交易成功
