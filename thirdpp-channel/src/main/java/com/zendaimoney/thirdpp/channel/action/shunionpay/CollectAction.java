...
import com.zendaimoney.thirdpp.channel.dto.req.shunionpay.collect.trade.CollectReq;
import com.zendaimoney.thirdpp.channel.dto.resp.RespDto;
import com.zendaimoney.thirdpp.channel.dto.resp.shunionpay.collect.trade.CollectResp;
 ...
 * 
 */
@ReqActionAnnotation(thirdType = ThirdType.SHUNIONPAY, bizType = BizType.BROKER_COLLECT, channelCategory = ChannelCategory.TRADE)
@Component("com.zendaimoney.thirdpp.channel.action.shunionpay.CollectAction")
public class CollectAction extends Action {
	...
	protected Message buildMessage(ReqDto dto, BizReqVo vo)
		...
		try {
			...
			// 报文请求地址
			message.setUrl(ConfigUtil.getInstance().getShunionpayConfig()
					.getCollectingUrl());

			// 组装签名前报文
			String signMessage = ShunionpayUtil
					.buildSignMessage(
							ShunionpayConstants.shunionpaycollectingSignPropertyNameArray,
							ShunionpayUtil.xmlToMap(dto.encode()));
			// 签名之后的报文
			String sign = ShunionpayUtil.Sign(signMessage, channelInfo);
			req.setChkValue(sign);
	...
	protected Response communicationSuccessHandler(RespDto dto,
		...
		// 业务逻辑处理
		// 响应码属于中间态：09、45
		String[] middleRetcode = Constants.ShunionpayConstants.RETCODE_STATE_MID
				.getCode().split(",");

		// 获取所有中间态
		String[] middleStatus = Constants.ShunionpayConstants.TRADE_STATE_MID.getCode().split(",");
		// 待更新
		...
		try {
			// header通道返回码
			String retCode = res.getResponseCode();

			// 通道返回信息
			String retMsg = res.getMessage();
			// 通道返回流水号
			String payTransFlow = "";

			// body通道返回码
			String bodyRetCode = res.getTransStat();
			...
			if (StringUtils.isEmpty(retCode) || Arrays.asList(middleRetcode).contains(retCode) ||
			...
			} else if (Constants.ShunionpayConstants.RESPONSE_CODE.getCode()
					.equals(retCode)
					&& Constants.ShunionpayConstants.TRADE_STATE_SUCC.getCode()
							.equals(bodyRetCode)) {
				// 返回状态为-交易成功
				...
				if (StringUtils.isBlank(retMsg)) {
					retMsg = Constants.ShunionpayConstants.TRADE_STATE_SUCC.getDesc();
				}
				// 交易记录状态
				...
				// transStat：1001 成功
				if (!Constants.ShunionpayConstants.TRADE_STATE_SUCC.getCode()
						.equals(bodyRetCode)) {
					retCode = bodyRetCode;
				}
				
				// 如果错误码为“未查询到符合条件的记录”需要特殊处理
				if (Constants.ShunionpayConstants.UNKNOW_RETURNCODE.getCode().equals(retCode)) {
					retMsg = Constants.ShunionpayConstants.UNKNOW_RETURNCODE.getDesc();
				}
				
				// 20TY响应码与描述 存在不一致的情况，需要特殊转换成：IP不通过
				if (Constants.ShunionpayConstants.UNVALID_IP_RETURNCODE.getCode().equals(retCode)) {
					retMsg = Constants.ShunionpayConstants.UNVALID_IP_RETURNCODE.getDesc();
				}
	...
	private void updateCollectMerId(ReqDto dto) {
		CollectReq req = (CollectReq)dto;
		CollectInfo collectInfo = new CollectInfo(req.getOrderNo(),req.getMerId());
		try {
