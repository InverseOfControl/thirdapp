...
import com.zendaimoney.thirdpp.channel.communication.unspay.UnspayCommunication;
...
import com.zendaimoney.thirdpp.channel.dto.resp.unspay.collect.trade.CollectResp;
 ...
 * 
 */
@ReqActionAnnotation(thirdType = ThirdType.UNSPAY, bizType = BizType.BROKER_COLLECT, channelCategory = ChannelCategory.TRADE)
@Component("com.zendaimoney.thirdpp.channel.action.unspay.CollectAction")
public class CollectAction extends Action {
	...
	@Autowired
	private UnspayCommunication unspayCommunication;
	...
	protected Message process(BizReqVo vo, ReqDto dto) throws PlatformException {
		...
		try {
			...
			if (Constants.MSG_IN_STORAGE_YES.equals(ConfigUtil.getInstance()
					.getUnspayConfig().getMsgInStorage())) {
		...
		try {
			...
			// message = unspayCommunication.sendMessageByHttp(message);
		} catch (Exception e) {
			logger.error("=================", e);
			// 设置http请求发送弄个异常状态码
	...
	protected Message buildMessage(ReqDto dto, BizReqVo vo)
		...
		try {
			...
			req = UnspayUtil.signMsg(req,vo,channelInfo);
			...
			// 请求参数对象Map
			Map<String, String> paraMap = UnspayUtil.xmlToMap(req.encode());
			String jsonstr = JSONHelper.bean2json(paraMap);
	...
	protected Response communicationSuccessHandler(RespDto dto,
		...
		try {
			...
			// 如果交易返回处于中间态.
			if (StringUtils.isEmpty(retCode) || Constants.UnspayConstants.RESPONSE_CODE.getCode().equals(retCode)) {
				// 返回状态为-交易中间态
	...
	private void updateCollectMerId(ReqDto dto) {
		CollectReq req = (CollectReq)dto;
		CollectInfo collectInfo = new CollectInfo(req.getDataContent().getOrderId(),req.getAccountId());
		try {
