...
import com.zendaimoney.thirdpp.channel.dto.req.allinpay.collect.query.v2.CollectQueryReq;
import com.zendaimoney.thirdpp.channel.dto.resp.RespDto;
import com.zendaimoney.thirdpp.channel.dto.resp.allinpay.collect.query.v2.RespBody;
import com.zendaimoney.thirdpp.channel.dto.resp.allinpay.collect.query.v2.CollectQueryResp;
 ...
 * 
 */
@ReqActionAnnotation(thirdType = ThirdType.ALLINPAY2, bizType = BizType.BROKER_COLLECT, channelCategory = ChannelCategory.QUERY)
@Component("com.zendaimoney.thirdpp.channel.action.allinpay.v2.CollectQueryAction")
public class CollectQueryAction extends Action {
	...
	protected Response communicationSuccessHandler(RespDto dto,
		...
		String[] middleStatus = Constants.AllinpayConstants.TRADE_STATE_QUERY_MID_V2.getCode().split(",");
		// 返回记录集合
		...
		try {
			...
			if (body != null && !body.isEmpty()) {
				...
				bodyRetCode = body.get(0).getRetCode();
				retMsg = body.get(0).getErrMsg();
				...
				if (StringUtils.isBlank(retCode) || StringUtils.isBlank(bodyRetCode)
					...
					if (Constants.AllinpayConstants.UNKNOW_RETURNCODE.getCode()
							.equals(retCode)) {
						response.setMsg(Constants.AllinpayConstants.UNKNOW_RETURNCODE
								.getDesc());
					} else {
				...
				if (StringUtils.isBlank(retCode) 
						|| Arrays.asList(middleStatus).contains(retCode) || Constants.AllinpayConstants.TRADE_STATE_SUCC.getCode().equals(retCode)) {
					// 返回状态为-交易中间态
					response.setCode(Constants.TppConstants.TRADE_STATE_MIDDLE
							.getCode());
				} else {
