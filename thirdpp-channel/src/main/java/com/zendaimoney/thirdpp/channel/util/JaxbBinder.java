...
public class JaxbBinder {
	 ...
	 * @param xmlText xml报文
	 * @param schemaName xsd文件名称
	 */
	public void doValidate(String xmlText, String schemaName) {
		/*try {
			SchemaFactory factory = SchemaFactory.newInstance(XMLConstants.W3C_XML_SCHEMA_NS_URI);
			File schemaFile = getschemaFile(schemaName);
			Schema schema = factory.newSchema(schemaFile);
			Document document = DocumentHelper.parseText(xmlText);
			Source source = new DocumentSource(document);
			schema.newValidator().validate(source);
		} catch (Exception e) {
			throw new PlatformException(PlatformErrorCode.XML_VALIDATE_ERROR, e);
		}*/
	}

/*	private File getschemaFile(String schemaName) throws IOException {
		String schemaTemplate = "schema/" + schemaName + ".xsd";
		Resource res = new ClassPathResource(schemaTemplate);
		return res.getFile();
	}*/
