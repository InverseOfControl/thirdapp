...
import com.zendaimoney.thirdpp.channel.dto.req.shunionpay.collect.query.CollectQueryReq;
import com.zendaimoney.thirdpp.channel.dto.resp.RespDto;
import com.zendaimoney.thirdpp.channel.dto.resp.shunionpay.collect.query.CollectQueryResp;
 ...
 * 
 */
@ReqActionAnnotation(thirdType = ThirdType.SHUNIONPAY, bizType = BizType.BROKER_COLLECT, channelCategory = ChannelCategory.QUERY)
@Component("com.zendaimoney.thirdpp.channel.action.shunionpay.CollectQueryAction")
public class CollectQueryAction extends Action {
	...
	protected Message buildMessage(ReqDto dto, BizReqVo vo)
		...
		try {
			...
			String signMessage = ShunionpayUtil
					.buildSignMessage(
							ShunionpayConstants.shunionpaycollecting_qrytransSignPropertyNameArray,
	...
	protected Response communicationSuccessHandler(RespDto dto,
		...
		// 获取所有中间态
		String[] middleStatus = Constants.ShunionpayConstants.TRADE_STATE_MID
				.getCode().split(",");
		...
		try {
			...
			if (StringUtils.isEmpty(retCode) || Arrays.asList(middleRetcode).contains(retCode) ||
				...
				// 0001-特殊处理,设置返回描述为:发送失败/未查询到符合条件的记录
