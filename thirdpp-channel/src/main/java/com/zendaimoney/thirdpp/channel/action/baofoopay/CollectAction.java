...
import com.zendaimoney.thirdpp.channel.dto.req.baofoopay.collect.trade.CollectReq;
 ...
 * 
 */
@ReqActionAnnotation(thirdType = ThirdType.BAOFOOPAY, bizType = BizType.BROKER_COLLECT, channelCategory = ChannelCategory.TRADE)
@Component("com.zendaimoney.thirdpp.channel.action.baofoopay.CollectAction")
public class CollectAction extends Action {
	...
	protected Message process(BizReqVo vo, ReqDto dto) throws PlatformException {
		...
		try {
			...
			if (Constants.MSG_IN_STORAGE_YES.equals(ConfigUtil.getInstance()
					.getBaofoopayConfig().getMsgInStorage())) {
	...
	protected Response respHandler(Message message, BizReqVo bizReqVo)
		...
		if (Constants.CommunicationStatus.HTTP_OK_STATUS.getCode().equals(
			...
			try {
				...
				logger.info("baofoopay collect response info:"+message.getResponseInfo());
	...
	protected Response communicationSuccessHandler(RespDto dto,
		...
		// 响应码属于成功：0000,BF00114
		String[] sucRetcode = Constants.BaofoopayConstants.TRADE_STATE_SUCC
				.getCode().split(",");

		// 获取所有中间态 BF00100,BF00112,BF00113,BF00115,BF00144,BF00202
		String[] middleStatus = Constants.BaofoopayConstants.TRADE_STATE_MID.getCode().split(",");
		// 待更新
		...
		try {
			...
			// 通道返回流水号
			String payTransFlow = res.getTransNo();

			// 如果交易返回处于中间态.
			if (StringUtils.isEmpty(retCode) || Arrays.asList(middleStatus).contains(retCode)) {
				// 返回状态为-交易中间态
			...
			} else if (Arrays.asList(sucRetcode).contains(retCode)) {
				// 返回状态为-交易成功
				...
				if (StringUtils.isBlank(retMsg)) {
					retMsg = Constants.BaofoopayConstants.TRADE_STATE_SUCC.getDesc();
				}
				// 交易记录状态
	...
	private void updateCollectMerId(ReqDto dto) {
		CollectReq req = (CollectReq)dto;
		CollectInfo collectInfo = new CollectInfo(req.getDataContent().getTransId(),req.getMerId());
		try {
