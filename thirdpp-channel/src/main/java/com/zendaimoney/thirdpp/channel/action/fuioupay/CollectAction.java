...
import com.zendaimoney.thirdpp.channel.dto.req.fuioupay.collect.trade.CollectReq;
import com.zendaimoney.thirdpp.channel.dto.resp.RespDto;
import com.zendaimoney.thirdpp.channel.dto.resp.fuioupay.collect.trade.CollectResp;
 ...
 *
 */
@ReqActionAnnotation(thirdType = ThirdType.FUIOUPAY, bizType = BizType.BROKER_COLLECT, channelCategory = ChannelCategory.TRADE)
@Component("com.zendaimoney.thirdpp.channel.action.fuioupay.CollectAction")
public class CollectAction extends Action {
	...
	protected Message process(BizReqVo vo, ReqDto dto) throws PlatformException {
		...
		try {
			...
			if (Constants.MSG_IN_STORAGE_YES.equals(ConfigUtil.getInstance()
					.getFuioupayConfig().getMsgInStorage())) {
	...
	protected Message buildMessage(ReqDto dto, BizReqVo vo)
		...
		Request request = null;
		CollectReq reqdto = (CollectReq) dto;
	...
	protected Response communicationSuccessHandler(RespDto dto,
		...
		// 业务逻辑处理
		// 获取所有中间态
		String[] middleStatus = Constants.FuioupayConstrans.TRADE_STATE_MID
				.getCode().split(",");
		// 待更新
		...
		try {
			...
			if (StringUtils.isBlank(retCode)
				...
				if (Constants.FuioupayConstrans.UNKNOW_RETURNCODE.getCode().contains(retCode)) {
					retMsg = Constants.FuioupayConstrans.UNKNOW_RETURNCODE.getDesc();
				}
				// 设置第三方通道返回错误信息
