...
import com.zdmoney.manager.common.enums.BizSys;
import com.zdmoney.manager.common.enums.BizType;
				...
				if (sourcePd != null && sourcePd.getReadMethod() != null) {
					try {
						...
						if (value != null || readMethod.getReturnType().getName().equals("java.lang.String")) {// 这里判断以下value是否为空，当然这里也能进行一些特殊要求的处理 例如绑定时格式转换等等，如果是String类型，则不需要验证是否为空
							...
							if(value instanceof BizType ||value instanceof ThirdType || value instanceof BizSys){
								isEmpty = true;
							}
							if (!isEmpty) {
