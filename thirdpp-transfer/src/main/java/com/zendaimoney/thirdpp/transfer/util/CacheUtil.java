...
public class CacheUtil {
	...
	public static void addAmount(ICacheClient cacheClient, CollectInfo collect) {
		...
		try {
			// 商户类型
			String merchantType = ThirdPPCacheContainer.sysInfoCategoryMap.get(collect.getInfoCategoryCode()).getMerchantType();

			SysThirdChannelInfo thirdChannel = ThirdPPCacheContainer.sysThirdChannelInfoMap.get(collect.getPaySysNo() + merchantType);

			// 通道编码+商户编码
			String key = collect.getPaySysNo() + "_"+ thirdChannel.getMerchantNo();
			if (cacheClient != null) {
