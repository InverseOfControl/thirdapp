...
function start(){
   ...
   tmp=`ps -ef | grep java| grep "com.zendaimoney.thirdpp.alarm"|grep $config_filepath | wc -l`  
   if [ $tmp -eq 1 ]; then
      if [ ! -f "$log_filepath" ]; then
         touch $log_filepath
      fi
      echo "tpp alarm already started..."
      exit 1
   fi 
   ...
   echo "tpp alarm is  starting..."

   LIB_JARS=`ls $LIB_DIR|grep .jar|awk '{print "'$LIB_DIR'/"$0}'|tr "\n" ":"`
   echo $LIB_JARS >"$log_filepath" &
   java $JAVA_MEM_OPTS -classpath $CONF_DIR:$CLASSES_DIR:$LIB_JARS com.zendaimoney.thirdpp.alarm.ServerStartup $1 $config_filepath > "$log_filepath" &
   appName=`grep "appName" $config_filepath | sed -e 's/.*=//'`
   echo "$!" > $BIN_DIR/"$appName".pid
   echo "tpp alarm start sucessful..." 
