...
import com.zendaimoney.thirdpp.account.exception.PlatformErrorCode;
...
public class Validate<T> {
	...
	public Response validate(T arg0, Class<T>... arg1) {
		...
		for (ConstraintViolation<T> constraintViolation : tempList) {
	 ...
	 *            Class<T>
	 */
	public Response validateProperty(T arg0, String propertyName, Class<T>... arg1) {
		Response response = new Response(Response.SUCCESS_RESPONSE_CODE);
		if (arg0 == null) {
			response = new Response(PlatformErrorCode.VALIDATE_ISNULL.getErrorCode(),
					"Object is null");
		...
		Set<ConstraintViolation<T>> constraintViolations = validator.validateProperty(arg0, propertyName, arg1);
		List<ConstraintViolation<T>> tempList = sort(constraintViolations);
		for (ConstraintViolation<T> constraintViolation : tempList) {
			message = constraintViolation.getMessage();
			if (message != null) {
