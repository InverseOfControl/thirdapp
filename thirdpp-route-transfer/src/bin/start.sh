...
function start(){
   ...
   tmp=`jps -ml| grep "com.zendaimoney.thirdpp.route.transfer"|grep $config_filepath | wc -l`  
   if [ $tmp -eq 1 ]; then
   ...
   java $JAVA_MEM_OPTS -classpath $CONF_DIR:$CLASSES_DIR:$LIB_JARS com.zendaimoney.thirdpp.route.transfer.ServerStartup $1 $config_filepath > "$log_filepath" &
