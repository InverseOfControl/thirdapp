...
import com.zendaimoney.thirdpp.trade.validate.rule.IsNeedSpilt;
...
public class CollectTask implements Serializable {
	...
	// 付款方证件类型
	// 付款人银行卡类型(0=身份证1=户口簿2=护照3=军官证4=士兵证5=港澳居民来往内地通行证6=台湾同胞来往内地通行证7=临时身份证8=外国人居留证9=警官证X=其他证件Y=驾驶证Z=回乡证)
	@NotBlank(message = "000001,为空", groups = InsertCheck.class)
	@Length(max = 1, message = "000002,过长")
	 ...
	 * 付款方手机号码
	 */
	@Length(max = 11, message = "000002,过长")
	...
	@NotNull(message = "000001,为空", groups = InsertCheck.class)
	@IsNeedSpilt(message = "000003,应该为 {0}{1}")
