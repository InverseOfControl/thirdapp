...
public class Constants {
	...
	public enum ChannelRulesConstants{
		CHANNEL_RULES_BIZSYS("1","按业务系统指定通道策略"),
		CHANNEL_RULES_BANKS("2","按银行指定通道策略"),
		CHANNEL_RULES_ROUTE("3","按路由规则指定通道策略")
		;
		private final String code;
		private final String desc;

		private ChannelRulesConstants(String code, String desc) {
			this.code = code;
			this.desc = desc;
		}
		...
		public static ChannelRulesConstants get(String code) {
			for (ChannelRulesConstants constants : ChannelRulesConstants.values()) {
				if (constants.getCode().equals(code))
					return constants;
			}
			throw new IllegalArgumentException("ChannelRulesConstants is not exist : "
					+ code);
		}
